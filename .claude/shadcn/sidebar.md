...
### Desktop-only sidebar (with BottomNavBar on mobile)

When the sidebar is desktop-only and mobile uses a separate BottomNavBar:

1. **Remove Sheet/mobile overlay** — The mobile Sheet overlay in `Sidebar` is unnecessary. The sidebar is `hidden md:block` via the wrapper, so mobile users never see it.
2. **Remove `useIsMobile` hook** — Not needed since the sidebar never renders on mobile.
3. **Layout split** — Use a desktop/mobile wrapper pattern in `layout.tsx`:
   ```tsx
   <body>
     ...
     {/* Mobile: Full-width + BottomNavBar */}
...
### Minimal dependencies (no cva, clsx, Sheet, Tooltip, Skeleton)

For projects that don't use the full shadcn stack:

- **Template literals** work fine for className merging instead of `cn()`/`clsx`/`tailwind-merge`.
- **No `cva`** — Replace `cva()` variant definitions with simple conditional className strings.
- **No Sheet** — Omit the mobile Sheet overlay entirely for desktop-only sidebars.
- **No Tooltip** — Use native `title` attribute instead of `Tooltip`/`TooltipProvider`.
- **No Skeleton** — Skip `SidebarMenuSkeleton` or create a simple CSS-based skeleton.
- **No Separator** — Skip `SidebarSeparator` or use a simple `<hr>`.
- **`Slot`** — Import from `@radix-ui/react-slot` for `asChild` prop support.

### Tailwind CSS v4 integration

1. Add sidebar CSS variables in `:root`:
   ```css
   --sidebar-background: #071a2e;
   --sidebar-foreground: #ededed;
   --sidebar-primary: #3b82f6;
   --sidebar-primary-foreground: #ffffff;
   --sidebar-accent: #0d2640;
   --sidebar-accent-foreground: #ededed;
   --sidebar-border: #153050;
   --sidebar-ring: #3b82f6;
   ```

2. Register them in `@theme inline` so Tailwind generates utility classes:
   ```css
   @theme inline {
   ...
   This enables classes like `bg-sidebar`, `text-sidebar-foreground`, `border-sidebar-border`, etc.
