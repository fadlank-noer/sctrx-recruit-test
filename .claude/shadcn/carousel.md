...
## Wheel Gestures Plugin (Scroll Navigation)

`embla-carousel-wheel-gestures` enables navigating slides via mouse wheel instead of drag.
...
**Options:**
- `wheelDraggingClass` (string) — CSS class added to carousel during wheel interaction.
- `forceWheelAxis` (`'x' | 'y'`) — Force a specific wheel axis to control navigation (e.g., vertical wheel scroll navigates a horizontal carousel).
- `target` (Element) — DOM element to attach wheel listener to (defaults to carousel container).

**Example — vertical carousel with wheel scroll:**
```tsx
<Carousel
  opts={{ align: "start" }}
  orientation="vertical"
  plugins={[WheelGesturesPlugin()]}
>
  <CarouselContent className="h-[135px]">
    <CarouselItem className="basis-full">...</CarouselItem>
  </CarouselContent>
</Carousel>
```
